...
#     )
# add_bg_from_local('nba-logo.jpg')
...
#Heat-Scatter PLot
if st.button('Heat-Scatter Plot'):
    st.header('Scatterplot Heatmap')
    df_selected_team.to_csv('output.csv',index=False)
    df = pd.read_csv('output.csv')
    corr_mat = df.corr().stack().reset_index(name="correlation")
    g = sns.relplot(data=corr_mat,x="level_0", y="level_1",
...
#     with sns.axes_style("white"):
#         f, ax = plt.subplots(figsize=(7, 5))
#         ax = sns.scatterplot(x=df['Tm'], y=df['PTS'],
#                             size=df['Tm'],data=df)
#     st.pyplot()